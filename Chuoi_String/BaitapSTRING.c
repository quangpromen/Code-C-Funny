...
void removeSpaces(char s[])// Ham xoa khoang trang ( Ham dung)
{
	int i=0,j=0;
	while(s[i]){
		if(s[i]!=' '){
...
/* void printReverseString(char s[]) //Ham dao nguoc(In theo thu tu nguoc lai)
 {
    int n= strlen(s);
    int i;
    for(i=n-1;i>=0;i--){
...
removeSpaces(s);
	...
	printf("Xoa ki khoang trang: %s",s);
	return 0;
}
